...
void ModLoader::LoadDllMods()
{
    ...
    for (size_t i = 0; i < dllMods.size(); i++)
    {
        ...
        else
        {
            ...
            MessageBox(NULL, std::string("Failed to load " + dllName).c_str(), "Anarchy Mod Loader", MB_OK | MB_ICONINFORMATION | MB_SYSTEMMODAL);
        }
    }
...
void ModLoader::OpenTerminal()
{
    if (AllocConsole())
    {
        ...
        SetWindowText(GetConsoleWindow(), "Anarchy Mod Loader");
    }
}
